...
základě každé jeho změny je spuštěna sestavovací sekvence. Při sestavení jsou poté spuštěny jednotkové a integrační testy.
Na jejich základě je možné zjistit případné chyby.
...
Kontrola zde porovnala průměr historických hodnot se získanými cenami. V případě, že rozdíl byl větší jak 25\%, byly ceny označené jako
možná chyba.
\par
...
\par
Je nutné podotknou, že spoustu špatných konstrukcí byly eliminovány již v průběhu implementace týmového projektu.
...
Pro větší přehlednost je třeba přesunout všechny servisní třídy vykonávající do samostatného balíku, které leží v nadřazeném balíčku 
...
velký problém při zpracování velkého počtu dat, především při nutnosti opakovaného stahování webových stránek. Při analýze kódu se ukázalo, že v aktuálním návrhu aplikace není možné tuto funkcionalitu jednoduše implementovat, aniž by se nejednalo o rychlou opravu, která může způsobit nestabilitu systému. Rychlá oprava by znamenala, že pro každou vytvářenou adresu je nutné se nejdříve pokusit najít vyřešenou chybu šablony 
nebo analyzátoru, ze které případně použít staženou stránku.
\par
Pro korektní opravu, která bude odpovídat čistého objektového návrhu, bylo tedy nejprve předělat celý proces vytváření požadavků.
Jelikož důvodů pro vytvoření požadavků z adresy může být více, je žádoucí chovat se při vytváření požadavků k adresám
různě.
\par
Výsledek hledání požadovaných adres, by měl mít striktně oddělené adresy dle toho jak byly nalezeny. Na základně typů poté
...
Po nalezení detailu produktu, jeho stažení v DataProvider modulu a následném vyparsování hodnot je třeba adresu spárovat s 
existující produktem. Důsledek nutnosti párování je, že po nalezení detailu není jisté, zda opravdu patří produktu, pro který 
byl detail vyhledán. 
\par
...
identifikátoru. Obchod také může poskytovat pouze název.
\par
V případě neúspěchu párovaní  možnost produkt spárovat manuálně, tedy po akci administrátora. Tento proces pak spoléhá na to, že vložena data
při vytváření kampaně jsou validní a neobsahují hodnoty, které by proces párování narušily, tak aby pároval chybně.
...
párování na jiných obchodech. Ukládání nových identifikátorů by mělo probíhat pouze se souhlasem administrátora, například 
při úspěšném párování nebo manuálním vložením.

\section{Uchování hodnot z nespárovaných adres}
Při vyhledávání na obchodu je zpravidla výsledek hledání větší množina adres, z nichž je většina nepotřebná. Pro dlouhodobě 
...
Kapitola realizace vylepšení se věnuje implementovaným vylepšení. Popisuje tak jaké změny byly provedeny. Většina těchto změn
byla zmíněna v návrhu vylepšení, které byly určeny v kontextu vytvořeného řešení. Bohužel ne všechny tyto změny byly dostačující
s dosažení běžícího systému. Proto byly v rámci implementace provedeny i změny se kterými se původně nepočítalo.
...
Ten obsahuje, zda proces skončil validně
...
\section{Plánování práce}
K tomuto účelu jsem navrhl rozhraní, které na základě adresy 
...
\section{Monitorování}
To lze dosáhnout pomocí monitorování, které poskytuje například služba DataDog \cite{dataDog}. Po instalaci hlavního monitorovacího programu na serveru, jsou všechny přístupné
informace odesílány na servery DataDog. Přes webové rozhraní je poté možné sledovat tyto údaje. Základní funkcionalita poskytuje
pouze informace o využití prostředků. Službu je však možné rozšířit o velký počet doplňků. Pomocí těch je pak možné sledovat
...
základní stavební kámen tohoto modulu jsem se rozhodl modul Finder rozdělit. Myšlenkou rozdělení bylo oddělení části, která vyhledává na internetovém obchodu a části, která samotné obchody, které by mohly produkt nabízet hledá. Důvod byl, že jsem se rozhodl implementovat pouze
první část, jelikož hledání samotných obchodů lze nahradit manuálním přidání konkurence na které chceme vyhledávat, případně 
využít některý se seznamů internetových obchodů v České republice a ty manuálně vložit.


\par
Nový návrh obsahoval odstranění modulu Finder a jeho příslušných front. Místo toho byl vytvořen modul nový, ProductDetailProvider.
Tento modul má za úkol zajišťovat hledání detailů produktu, což je dosaženo na základě šablony pro daný e-shop, která obsahuje 
tyto atributy:
\begin{itemize}

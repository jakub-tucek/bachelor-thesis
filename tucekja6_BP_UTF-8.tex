...
\par
Druhá změna obsahuje změna rozhraní, kdy bylo potřeba
zmenšit počet parametrů a odstranit výjimku, která byla vyhozena v případě nalezení chyby. 
\par
Třetí změna je poté samotné pouštění 
validací. V původním řešení byla třída závislá na všech příslušných validací, které spouštěla a navíc skončila při první chybě, což je jedna 
z příčin chování, které jsem popsal v kapitole poukazující na možnosti spojit chyby analyzátoru \autoref{analyser-join}
\par
Vytvořil jsem nový návrh, který je pak použit i na ostatních místech interní části v závislosti na
prováděných vylepšení. Servisní třídě jsem odebral jednotlivé závislosti na validacích a nahradil je množinou 
...
\par
Validačnímu rozhraní byl změněn návratový typ na \textit{Optional} případné chyby
nebo prázdnou hodnotu \textit{Optional}. Implementace těchto rozhraní pak zahrnovala kontrolu jednotlivých hodnot.
...
pomocí abstraktní a generické třídy \textit{AbstractAnalysis}. Vlastnosti skupin pak obsahují třídy \textit{AbstractStringAnalysis} 
a \textit{AbstractPriceAnalysis}. Výsledná základní validace kontrolující, zda získaná hodnota odpovídá nějaké historické hodnotě
vypadá následovně:

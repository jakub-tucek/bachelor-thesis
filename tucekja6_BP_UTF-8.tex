...
Stránka se tak znova redundantně stáhla.
...
požadavků. K tomu je spojený celý proces párování detailu produktu. Proces párování, pak nebyl implementován ani jak v samotném modulu
ProductProvider, tak s ním nebylo počítáno při plánování práce.
\par
...
je velký problém oprava nebo implementace jakýkoliv nových požadavků. Systém proto v tomto stavu
není vhodný pro následný rozvoj z důvodu velké časové investice.

\chapter{Návrh na vylepšení}
V této kapitole popíšu návrh na vylepšení a samotný důvod, proč považuji tyto změny za důležité.

\section{Pojmy}

\subsection{Mock}
V objektově orientovaném programování se Mock objekt požívá pro simulování reálné třídy\cite{mock}.
Při testování tak není třída závislá na implementaci tříd na kterých je závislá, ale pouze na instanci Mock objektu, který
se chová podle nedefinovaného chování. Dále Mock objekt uchovává všechny na něho provedené volání, díky čemuž je možné
testovat vnitřní chování a ne pouze návratovou hodnotu.

\section{Oprava komunikace Manager - ProductProvider}
Jak bylo zmíněno výše bylo třeba komunikaci existujících dvou modulů optimalizovat. Neefektivní chování je
...
systému. Tento problém je spojoval s tím, že je velmi obtížné pokrýt jednotkovými testy. Navíc tyto testy v 

...
Návrh byl podrobně zmíněn v kapitole \ref{ch:architecture-tests} a proto
bude zmíněn v následujícím přehledu pouze bodově.

\par

Na základě shrnutí nedostatků v tabulkách \ref{table:analysis-old1} a \ref{table:analysis-old2}je možné prohlásit, že pro základní použití systému je především nutná oprava všech kritických chyb a doplnění o chybějící funkcionalitu. Pro zajištění jednoduchého zapracování změn bude také nutný refaktoring, úprava návrhu a doplnění základních testů, které chybějí.
Statická analýza kódu ukázala, že počet chyb je minimální, což bylo způsobeno jejím průběžným použítím při vývoji.
\par
Problémy týkající se nedostatečné přívětivosti administrátorského rozhraní, způsobují nepohodlné použití systému a bylo by vhodně je též zapracovat.
\par
V případě opravy výše uvedených nedostatků předpokládám
jednoduchou možnost oprav neefektivního chování, které se může negativně odrazit při velké zátěži. 
Škálovatelnost má vzhledem k ostatním problémům nejnižší prioritu a není nepovažována narozdíl od ostatních bodů za nutnou k zapracování.
